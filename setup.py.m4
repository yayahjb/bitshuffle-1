changequote(`<<<',`>>>')dnl
ifelse(packageprefix,<<<>>>,dnl
<<<define(<<<packageinclude>>>,<<<>>>)>>>dnl
<<<define(<<<packagelib>>>,<<<>>>)>>>dnl
,<<<define(<<<packageinclude>>>,<<<os.path.join(>>>'packageprefix'<<<,'include')>>>)>>>dnl
<<<define(<<<packagelib>>>,<<<os.path.join(>>>'packageprefix'<<<,'lib')>>>)>>>)dnl
...
INCLUDE_DIRS = [>>>packageinclude<<<]
LIBRARY_DIRS = [>>>packagelib<<<]
